...
class oxTestModuleLoader
{
     ...
     *
     * @param array $aDependencies a list with paths relative to the base module dir
     */
    public static function addDependencies($aDependencies)
    {
        $sErrors = "";
        $sBaseModuleDir = oxRegistry::getConfig()->getConfigParam("sShopDir") . "/modules/";

        foreach ($aDependencies as $sPath) {
            if (file_exists($sBaseModuleDir . $sPath . "/metadata.php")) {
                self::_initMetadata($sBaseModuleDir . $sPath . "/metadata.php");
            } else {
                $sErrors .= "Unable to find metadata file in directory: $sPath" . PHP_EOL;
            }
        }
     ...
     */
    private static function _initMetadata($sPath)
    {
        include $sPath;
        ...
        if (isset($aModule["files"]) && count($aModule["files"])) {
            foreach ($aModule["files"] as $sFilePath) {
                ...
                if (!class_exists($sClassName, false)) {
                    require oxRegistry::getConfig()->getConfigParam("sShopDir") . "/modules/bepado/" . $sFilePath;
                }
            }
        }
        ...
        if (isset($aModule["extend"]) && count($aModule["extend"])) {
            foreach ($aModule["extend"] as $sParent => $sPathToModule) {
                ...
                require_once oxRegistry::getConfig()->getConfigParam("sShopDir") . "/modules/bepado/" . $sPathToModule . ".php";
            }
